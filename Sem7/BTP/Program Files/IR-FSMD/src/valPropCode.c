...
boolean myCorrespondenceChecker( NODE_ST *q0, NODE_ST *q1, PATHS_LIST *P0, PATHS_LIST *P1, FSMD *M0, FSMD *M1, CPS_Q *correspondingStatePairs, CPS_Q *C_correspondingStatePairs, PATH_PAIR *uncondEquivalent, PATH_PAIR* condEquivalent, PATH_PAIR* debugList ){
    if(M0 == NULL && M1 == NULL) return TRUE;
    ...
    if(q0==(NODE_ST*)NULL || q1==(NODE_ST*)NULL) {
        printf("\n In correspondenceChecker() with a NULL state\nERROR: Exiting System\n");
        exit(0);
    }
    if(P0==(PATHS_LIST*)NULL || P1==(PATHS_LIST*)NULL) {
    ...
    if(correspondingStatePairs == (CPS_Q*)NULL) {
        printf("\n In correspondenceChecker() with a NULL correspondingStatePairs \nERROR: Exiting System\n");
        exit(0);
    }
    if(uncondEquivalent == (PATH_PAIR*)NULL || condEquivalent == (PATH_PAIR*)NULL) {
        printf("\n In correspondenceChecker() with a NULL E\nERROR: Exiting System\n");
        exit(0);
    }
    if(debugList == (PATH_PAIR*)NULL) {
        printf("\n In correspondenceChecker() with a NULL E\nERROR: Exiting System\n");
        exit(0);
    }
    indexP0 = (int*)malloc(sizeof(int));
    indexP1 = (int*)malloc(sizeof(int));

//#ifdef DEbug
    printf("\nIn correspondenceChecker ( %s, %s )\n", q0->state_id, q1->state_id);
//#endif

    for(int i = 0 ; i < P0->numpaths; i++){

        //check to find which paths emanate from q0
        if( strcmp(M0->states[P0->paths[i].start].state_id, q0->state_id) == 0 ) {
            //Feasibility check for the condition
...
void securityChecker(FSMD* M0, FSMD* M1){

    ///////////     NULL CHECKS     //////////
    if(M0 == NULL || M1 == NULL) {
        printf("Security Checker: M0 or M1 is NULL\n");
        return;
    }

    ///////////     VARIABLE DECLARATION START    //////////
    PATHS_LIST pathCover0, pathCover1;                              // store path covers pathCover0 for M0 and vice-versa
    CPS_Q correspondingStatePairs, C_CorrespondingStatePairs;       // set corresponding and C_corresponding state pairs
    CPS deqCorrespondingStatePairs;                                 // to deque corresponding state pairs
    PATH_PAIR *condEquivalent, *uncondEquivalent;                   // to store conditionally and unconditionally equivalent path pairs
    PATH_PAIR *debugList;                                           // stores (potential) C-equivalent paths along a concatenated path; In case of failure, this is printed for debug
    struct rusage timeUsage;
    boolean result;
    ///////////     VARIABLE DECLARATION END     //////////

    cal_V0_intersection_V1( &V0_V1 );
    printf("\nCommon variables in M0, M1 : \n");
    for(int i = 0 ; i < V0_V1.no_of_elements; i++) printf("%s ", V0_V1.var_string[i]);

    cal_V1_minus_V0_intersection_V1(&V1_minus_V0_V1);
    printf("\n\nVariables in M1 and not in M0 : \n");         // these can cause security leak, but not the other way around
    for(int i = 0 ; i < V1_minus_V0_V1.no_of_elements; i++) printf("%s ", V1_minus_V0_V1.var_string[i]);

    ///////////     DS INITIALIZATION START     //////////
    initP(&correspondingStatePairs);
    enQP(&correspondingStatePairs,0,0);                             // place the reset states of two fsmds as corresponding state pair in correspondingStatePairs
    initializeStatePairVisited( corresponding_state_pair_visited );

    initP(&C_CorrespondingStatePairs);
    initializeStatePairVisited( C_corresponding_state_pair_visited );

    uncondEquivalent = initE();
    condEquivalent = initE();

    findpaths(M0,&pathCover0);
    Associate_R_and_r_alpha(&pathCover1);                           // computes R_alpha & r_alpha

    findpaths(M1,&pathCover1);
    Associate_R_and_r_alpha(&pathCover1);                           // TODO: what does this do?
    ///////////     DS INITIALIZATION END     //////////

#ifdef LVA
    performLiveVariableAnalysis(&pathCover0);                       // print live Variables?
    performLiveVariableAnalysis(&pathCover1);                       // print live Variables?
#endif

#ifdef DEbug
    printf("\n\nSymbol Table : \n");
    for(int i=0; i < stab.numsymbols; i++)
    ...
    while( !(emptyP(&correspondingStatePairs)) ){
        ...
        resetPropagatedVector( &(M0->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s0].propVector) );
        resetPropagatedVector( &(M1->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s1].propVector) );

        //TODO: correspondenceChecker called here myCorrespondenceChecker
        if(myCorrespondenceChecker(&(M0->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s0]), &(M1->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s1]), &pathCover0, &pathCover1, M0, M1, &correspondingStatePairs, &C_CorrespondingStatePairs, uncondEquivalent, condEquivalent, debugList) == FALSE){
//            if(correspondenceChecker(&(M0->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s0]), &(M1->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s1]), &pathCover0, &pathCover1, M0, M1, &correspondingStatePairs, &C_CorrespondingStatePairs, uncondEquivalent, condEquivalent, debugList) == FALSE){
            printf("\n\nFailure detected in case of correspondenceChecker( %s, %s )\n\n",M0->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s0].state_id, M1->states[correspondingStatePairs.cstate[correspondingStatePairs.front].s1].state_id);
            result = FALSE;
            break;
        }

        deQP(&correspondingStatePairs, &deqCorrespondingStatePairs);
    }
...
//    printf("\n Maximum mismatch between two value vectors: %d\n", MAX_MISMATCH);
    ...
    printEta(&correspondingStatePairs, M0, M1, corresponding_state_pair_visited);

    printf("\n\n List of C-corresponding states: \n");
    printEta(&C_CorrespondingStatePairs, M0, M1, C_corresponding_state_pair_visited);

    printf("\n\n\n List of unconditionally equivalent (U-equivalent) paths: \n");
    printE(uncondEquivalent, &pathCover0, &pathCover1, M0, M1);

    printf("\n\n List of conditionally equivalent (C-equivalent) paths: \n");
    printE(condEquivalent, &pathCover0, &pathCover1, M0, M1);
