...
stack_push( STACK *S,int state,NC *cond, DATA_TRANS *action) {
	...
	if(S!=NULL) {
		if(S->top>=STACK_SIZE) { // checking for stack over flow
			printf("\nStack over flow \n ");
			exit(0);
		}
		else {
...
/*      This function will find whether a state is a starting node for a cycle         */

/*****************************************************************************/

enum COLOR{WHITE, BLACK, GREY};

void findCycles(FSMD* fsmd, int stateId, int parent, enum COLOR* color, int* mark, int* par, int* cycleCount){
    if(color[stateId] == BLACK || fsmd == NULL) return;
    if(color[stateId] == GREY){
        (*cycleCount++);
        int cur = parent;
        mark[cur] = *cycleCount;
    ...
    int transitionCount = fsmd->states[stateId].numtrans;
    if(transitionCount > 0) {
        int nextState = fsmd->states[stateId].translist->outtrans;
        if(nextState != par[stateId]) {
            findCycles(fsmd, nextState, stateId, color, mark, par, cycleCount);
        }
    }
    if(transitionCount > 1){
        int nextState = fsmd->states[stateId].translist->next->outtrans;
        if(nextState != par[stateId]) {
            findCycles(fsmd, nextState, stateId, color, mark, par, cycleCount);
        }
    }
...
void setIsCycle(FSMD* fsmd, int mark[], int parent[]){
    if(fsmd == NULL) return;
    for(int i = 0 ; i < fsmd->numstates; i++){
        if(mark[i] != 0 && ((parent[i]!= -1 && mark[parent[i]] != mark[i]) || fsmd->states[i].numtrans > 1)) fsmd->states[i].inCycle = TRUE;
//        if(mark[i] != 0 && fsmd->states[i].numtrans > 1) fsmd->states[i].inCycle = TRUE;
        if(parent[i] == -1) printf("%s - \tparent - %d, \tcycle No : %d\n", fsmd->states[i].state_id, parent[i], mark[i]);
        else printf("%s - \tparent - %s, \tcycle No : %d\n", fsmd->states[i].state_id, fsmd->states[parent[i]].state_id, mark[i]);
    }
}
void findAllCutpoints(FSMD *fsmd) {
    if(fsmd == NULL) return;
    enum COLOR color[MAX_STATES] = {0};
    int mark[MAX_STATES] = {0};
    int parent[MAX_STATES];
    int cycleCount = 0;
    for (int i = 0;  i < MAX_STATES; i++) {
        fsmd->states[i].inCycle = FALSE;
        parent[i] = -1;
    }
    findCycles(fsmd, 0, -1, color, mark, parent, &cycleCount);
    setIsCycle(fsmd, mark, parent);
}
...
cutpoint( NODE_ST state ) {
    if( state.node_type == 0 || state.node_type == 1 ||  state.inCycle == TRUE)
        return TRUE;
    else
        return FALSE;
}
